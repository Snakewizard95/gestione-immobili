...
export function analizzaRendimentiAffitti(buffer: ArrayBuffer): RisultatoAnalisi {
  ...
  for (const r of righeRaw) {
    const [prop, imm, cond, aff, condominio, mutuo, imposta, imu, , valore] = r as unknown[]
    ...
    righe.push({
      ...
      affitto_cent: centesimi(aff), condominio_cent: centesimi(condominio), mutuo_cent: centesimi(mutuo),
      imposta_registro_cent: centesimi(imposta), imu_cent: centesimi(imu), valore_mercato_cent: centesimi(valore),
    })
  }
  ...
  for (const r of righe) {
    ...
    if (imm) {
      ...
      imm = {
        ...
        valore_mercato_cent: r.valore_mercato_cent || null, imu_annua_cent: r.imu_cent || null,
        mutuo_annuo_cent: r.mutuo_cent || null, condominio_annuo_cent: r.condominio_cent || null, note: 'Importato da Excel',
      }
    ...
    piano.contratti.push({
      ...
      periodicita: 'mensile', giorno_scadenza: 5, deposito_cent: null, deposito_modalita: '', deposito_restituito_il: '', regime_iva: '',
